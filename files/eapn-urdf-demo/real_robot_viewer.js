...
(() => {
  ...
  section.innerHTML = `<div class="case-heading"><div><h3>R1 · Bimanual cloth folding</h3><small>AGILEX-PIPER · enhanced display · original timing · 30 fps</small></div><span class="real-badge">REAL ROBOT</span></div>
    ...
    <div class="real-downloads">${clips.map(c => `<div><b class="real-${c.method}">${methodLabel(c)}</b><a href="${escape(c.original_src || c.src)}" download>Original video</a><a href="${escape(c.csv)}" download>Joint CSV · ${c.joints.rows} frames</a></div>`).join('')}</div>`;
